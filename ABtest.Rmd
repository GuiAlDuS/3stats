...
Utilizando como única variable el ancho del sépalo (sepal width), ¿es la distribución de esa variable en  *versicolor* distinta a la de una especie ficticia formada por la mezcla de las especies *virginica* y *setosa*?

Creemos esa otra especie seleccionado aleatoriamente 50 filas de la combinación de *virginica* y *setosa*. De esta manera nuestras dos muestras tendrán el mismo tamaño. 
...
La distribución de las permutaciones apenas logró acercarse a la diferencia medida entre *versicolor* y nuestra *otra especie*. Podemos ver esto más claramente con el resumen de la distribución de las permutaciones:

```{r}
summary(permut_results)
```

Según el histograma y resumen de la distribución, vemos que algunas de las permutaciones sí lograron superar el valor medido entre los promedios. Esto lo podemos cuantificar facilmente:

```{r}
length(permut_results[permut_results < diff(diference$mean)])
```

Si tomamos en cuenta que únicamente 4 permutaciones de un total de 100000 lograron alcanzar la diferencia medida, podemos decir entonces que existe una probabilidad muy baja (menor a 1/25000) de que la diferencia medida entre los promedios de *versicolor* y *otra especie* se produzca al azar si ambas distribuciones pertenecieran a una sola especie. 

En otras palabras, tenemos certeza de que ambas muestras provienen de distribuciones distintas.
...
analysis_classified <- analysis %>% 
  mutate(sp_id = case_when(Species == "versicolor" ~ as.integer(1),
...
Para esto debemos asignar un índice distinto a cero para cada especie y crear un modelo donde cada una de las especies tendrá una intersección distinta. Para esta índice damos a *Versicolor* un valor igual a 2 y *otra especie* de 1.

Notese que para el paquete Rethinking, los números asignados a estos índices deben de ser enteros.

```{r message=FALSE, warning=FALSE, results=FALSE, collapse=TRUE}
an_reth2 <- analysis %>% 
  mutate(Sp = as.integer(case_when(Species == "versicolor" ~ 2,
...
Se puede notar que las distribuciones de **a1** (distribución del promedio de *otra especie*) y **a2** (distribución del promedio de *Versicolor*) no se sobreponen. En el caso de **a1** el 95% de la distribución se encuentra entre 2.97 y 3.19, mientras que para **a2** va de 2.68 a 2.86.
...
```{r message=FALSE, warning=FALSE, results=FALSE, collapse=TRUE}
mod_brms_frm <- bf(SepalWidth ~ Species, sigma ~ Species)
